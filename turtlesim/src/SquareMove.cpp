...
private:
    ...
    bool reached_goal_()
    {
        return fabsf(pose_.x - goal_.x) < 0.1 && fabsf(pose_.y - goal_.y) < 0.1 &&
    ...
    void print_goal_()
    {
        RCLCPP_INFO(rclcpp::get_logger("turtlesim_square_move"), "New goal [%f %f, %f]", goal_.x, goal_.y, goal_.theta);
    }
